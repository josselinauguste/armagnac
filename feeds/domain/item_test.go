...
func TestNewItemFromRss(t *testing.T) {
	...
	assert.Equal(t, rssItem.Description, rssItem.Description)
...
func TestMappingItemDescriptionFromContent(t *testing.T) {
	rssItem := rss.Item{
		Title:   "Le titre",
		Content: &rss.Content{Text: "La description"},
		Author:  rss.Author{Name: "Josselin Auguste"},
		PubDate: "LOL, 10 Jul 2015 09:33:50 +0000",
		Links:   []*rss.Link{&rss.Link{Href: "http://www.salut.com/le-titre"}},
	}
	...
	assert.NotNil(t, item)
	assert.Equal(t, rssItem.Content.Text, item.Description)
}

func TestGetExcerpt(t *testing.T) {
	item := Item{Description: "Description"}

	assert.Equal(t, item.Description, item.Excerpt())
}
